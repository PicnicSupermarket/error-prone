...
public class IsParentReturnTree implements Matcher<ExpressionTree> {
  ...
  public boolean matches(ExpressionTree expressionTree, VisitorState state) {
    ...
    // XXX: Quick fix for the direct return cases. Perhaps it is a small performance win?
    if (state.getPath().getParentPath().getLeaf() instanceof BlockTree
        && state.getPath().getParentPath().getParentPath().getLeaf() instanceof MethodTree) {
      return true;
    }
